...
use ethstore::dir::{KeyDirectory, allchainDirectory, DiskDirectory, GethDirectory, DirectoryType};
...
Options:
    ...
    --dir DIR          Specify the secret store directory. It may be either
                       allchain, allchain-test, geth, geth-test
                       or a path [default: allchain].
    --src DIR          Specify import source. It may be either
                       allchain, allchain-test, get, geth-test
...
fn key_dir(location: &str) -> Result<Box<KeyDirectory>, Error> {
	let dir: Box<KeyDirectory> = match location {
		"allchain" => Box::new(try!(allchainDirectory::create(DirectoryType::Main))),
		"allchain-test" => Box::new(try!(allchainDirectory::create(DirectoryType::Testnet))),
